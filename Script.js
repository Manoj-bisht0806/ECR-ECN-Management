...
async function loadSharedData(){
 if(!C.apiUrl || sharedSyncing) return;
 sharedSyncing=true;
 try{
   const res=await fetch(C.apiUrl+'?action=get&t='+Date.now(),{cache:'no-store'});
   if(!res.ok) throw new Error('Shared server unavailable');
   const payload=await res.json();
   const server={ecr:Array.isArray(payload.ecr)?payload.ecr:[],ecn:Array.isArray(payload.ecn)?payload.ecn:[]};
   if(server.ecr.length || server.ecn.length){
     data=server; localStorage.setItem(C.storageKey,JSON.stringify(data));
   }else if(data.ecr.length || data.ecn.length){
   ...
   console.warn('Shared data sync unavailable:',e.message);
 }finally{sharedSyncing=false;}
}
async function pushAllToServer(){
 if(!C.apiUrl) return;
 const res=await fetch(C.apiUrl,{method:'POST',headers:{'Content-Type':'text/plain;charset=utf-8'},body:JSON.stringify({action:'bulk',data})});
 if(!res.ok) throw new Error('Could not upload local data');
 const payload=await res.json();
 if(payload.ecr||payload.ecn) data={ecr:payload.ecr||[],ecn:payload.ecn||[]};
 localStorage.setItem(C.storageKey,JSON.stringify(data));
}
async function syncRecordToServer(type,record){
 if(!C.apiUrl) return;
 try{
   const res=await fetch(C.apiUrl,{method:'POST',headers:{'Content-Type':'text/plain;charset=utf-8'},body:JSON.stringify({action:'save',type,id:record.id,record})});
   if(!res.ok) throw new Error('save failed');
 }catch(e){toast('Saved in this browser, but shared server update failed','error');}
}
async function deleteRecordFromServer(type,id){
 if(!C.apiUrl) return;
 try{await fetch(C.apiUrl,{method:'POST',headers:{'Content-Type':'text/plain;charset=utf-8'},body:JSON.stringify({action:'delete',type,id})});}catch(e){toast('Deleted locally, but shared server update failed','error');}
}
...
function dashboard(){
 ...
 <div class="kpis">
  ${kpi("Total ECR",data.ecr.length,"blue","go('ecr')")}${kpi("Open ECR",eo,"green","state.filterStatus='Open';go('ecr')")}${kpi("Closed ECR",ec,"orange","state.filterStatus='Closed';go('ecr')")}${kpi("Rejected ECR",er,"red","state.filterStatus='Rejected';go('ecr')")}
  ${kpi("Total ECN",data.ecn.length,"blue","go('ecn')")}${kpi("Open ECN",no,"green","state.filterStatus='Open';go('ecn')")}${kpi("Closed ECN",nc,"orange","state.filterStatus='Closed';go('ecn')")}${kpi("Rejected ECN",nr,"red","state.filterStatus='Rejected';go('ecn')")}
 </div>
 <div class="grid2">
  <div class="panel"><h3>ECR Status</h3><div class="chart-wrap">${donutHtml(data.ecr.length,eo,ec,er,"ECR")}</div></div>
  <div class="panel"><h3>ECN Status</h3><div class="chart-wrap">${donutHtml(data.ecn.length,no,nc,nr,"ECN")}</div></div>
 </div>
 <div class="dashboard-latest-stack">
  <div class="dashboard-latest-section">${recentPanel("Latest SIEL ECR",latestE,"ECR")}</div>
  <div class="dashboard-latest-section">${recentPanel("Latest SIEL ECN",latestN,"ECN")}</div>
 </div>`;
}
function donutHtml(totalCount,open,closed,rejected,label){
 const pct=(v,t)=>t?((v/t)*100).toFixed(1):0;
 if(!totalCount) return `<div class="empty-chart"><div class="donut zero"><span>0</span></div><div class="legend"><div><i class="dot" style="background:var(--blue)"></i>Open 0 (0%)</div><div><i class="dot" style="background:var(--green)"></i>Closed 0 (0%)</div><div><i class="dot" style="background:var(--red)"></i>Rejected 0 (0%)</div></div></div>`;
 return `<div class="chart-wrap"><div class="donut" style="--open:${open/totalCount*100}%;--closed:${(open+closed)/totalCount*100}%"><span>${totalCount}</span></div><div class="legend"><div><i class="dot" style="background:var(--blue)"></i>Open ${open} (${pct(open,totalCount)}%)</div><div><i class="dot" style="background:var(--green)"></i>Closed ${closed} (${pct(closed,totalCount)}%)</div><div><i class="dot" style="background:var(--red)"></i>Rejected ${rejected} (${pct(rejected,totalCount)}%)</div></div></div>`;
}
...
function entryPage(){
 ...
 ["localImport","Local/Import","local",false],["pa","PA","text",false],["partcode","Partcode","text",false],["priority","Priority","priority",false],["remarks","Remarks","textarea",false]
 ];
...
function fieldHtml(key,label,type,req,val=""){
 ...
 else if(type==="priority") el=`<select class="select" name="${key}"><option value="">Select Priority</option>${priorities.map(x=>`<option ${val===x?"selected":""}>${x}</option>`).join("")}</select>`;
...
function submitEntry(ev){
 ...
 if(!r.status) r.status="Open"; if(!r.priority) r.priority="C";
 const list=all(state.activeEntry);
 let savedRecord=null;
 if(state.editId){const i=list.findIndex(x=>x.id===state.editId);if(i>=0){list[i]={...list[i],...r};savedRecord={...list[i]};}}
 else {r.id=uid(state.activeEntry);r.createdAt=new Date().toISOString();list.unshift(r);savedRecord={...r};}
 saveLocal(true); state.editId=null; renderPage();
 if(savedRecord) syncRecordToServer(state.activeEntry,savedRecord); maybeSyncExcel();
}
...
function deleteRecord(type,id){
 ...
 const list=all(type), i=list.findIndex(x=>x.id===id); if(i>=0){list.splice(i,1);saveLocal();renderPage();deleteRecordFromServer(type,id);maybeSyncExcel();}
}
