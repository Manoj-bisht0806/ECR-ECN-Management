...
const state = { page: "dashboard", filterStatus:null, filterType:null, search:"", editId:null, admin:false, activeEntry:"ECR", reportFrom:"2026-09-01", reportTo:"2026-09-30", excelHandle:null };
...
function seedData(){
  ...
  // Seed data is loaded from Ecr.json if fetch is allowed; fallback embedded sample.
  return window.SEED_DATA || {ecr:[],ecn:[]};
}
let data = seedData();

async function loadSeedIfEmpty(){
  if(data.ecr.length || data.ecn.length) return;
  try{
    const r=await fetch("Ecr.json"); if(r.ok){ data=await r.json(); saveLocal(false); }
  }catch(e){}
}
...
(async()=>{await loadSeedIfEmpty();render()})();
